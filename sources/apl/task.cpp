...
namespace apl
{
    ...
    task_environment::task_environment(): this_task_id({{0, MESSAGE_SOURCE::GLOBAL}, 0, TASK_SOURCE::GLOBAL}), processes_count(0), threads_count(0)
    { }

    task_environment::task_environment(task_data& td): this_task_id({{0, MESSAGE_SOURCE::GLOBAL}, 0, TASK_SOURCE::GLOBAL}), processes_count(0), threads_count(0)
    {
        this_task = td;
        set_all_task_data();
    }
    ...
    task_environment::task_environment(task_type type, size_t args_count, size_t const_args_count, size_t _processes_count, size_t _threads_count): this_task_id({ {0, MESSAGE_SOURCE::GLOBAL}, 0, TASK_SOURCE::GLOBAL }), processes_count(_processes_count), threads_count(_threads_count)
    {
        ...
        for (size_t j = 0; j < const_args_count; ++j)
    ...
    void task_environment::set_workers_count(size_t _processes_count, size_t _threads_count)
    {
    ...
    { return processes_count * threads_count; }

    size_t task_environment::get_processes_count() const
    { return processes_count; }

    size_t task_environment::get_threads_count() const
    ...
    size_t task::get_processes_count() const
    { return env->get_processes_count(); }

    size_t task::get_threads_count() const
    { return env->get_threads_count(); }
